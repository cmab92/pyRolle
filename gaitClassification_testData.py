...
class Two_class_svm:
    ...
    def extract_features(self, data):
        ...
        if self.__num_coefficients != 0:
            for i in range(self.__num_of_used_vars):
                ...
                c_a, c_d = pywt.dwt(data[::, i], wavelet=self.__wavelet, mode='symmetric')
                # extract maximum approx. coefficients
                coefficients = c_a
                if self.__wavelet_stats:
                    feature_vector.append(mad(coefficients))
                    feature_of_variable.append(i)
                coefficients_amp.append(coefficients[coefficients.argsort()[-self.__num_coefficients:]])
                ...
                if np.max(coefficients) == 0:
                    ...
                    coefficients_val.append(translation_axis[coefficients.argsort()[-self.__num_coefficients:]])
                for k in range(np.size(coefficients_val[i])):
                    temp = coefficients_val
                    feature_vector.append(temp[i][k])
                    feature_of_variable.append(i)
                for k in range(np.size(coefficients_amp[i])):
                ...
                for j in range(self.__num_levels - 1):
                    c_a, c_d = pywt.dwt(c_a, wavelet=self.__wavelet, mode='symmetric')
                    # extract maximum approx. coefficients
                    coefficients = c_a
                    if self.__wavelet_stats:
                        feature_vector.append(mad(coefficients))
    ...
    def dump_all(self):
        ...
        with open("dumps/gait_clf_two_class_2.pkl", 'wb') as clf_dump:
            cPickle.dump(self.__clf, clf_dump)

        with open("dumps/gait_sel_var_two_class_2.pkl", 'wb') as param_dump:
            cPickle.dump(self.__selected_variables, param_dump)

        with open("dumps/gait_feat_val_two_class_2.pkl", 'wb') as param_dump:
            cPickle.dump(self.__feature_validity, param_dump)

        with open("dumps/gait_mue_two_class_2.pkl", 'wb') as param_dump:
            cPickle.dump(self.__std_mue, param_dump)

        with open("dumps/gait_sigma_two_class_2.pkl", 'wb') as param_dump:
            cPickle.dump( self.__std_sigma, param_dump)

    def load_all(self):

        with open("dumps/gait_clf_two_class_2.pkl", 'rb') as clf_dump:
            self.__clf = cPickle.load(clf_dump)

        with open("dumps/gait_sel_var_two_class_2.pkl", 'rb') as param_dump:
            self.__selected_variables = cPickle.load(param_dump)

        with open("dumps/gait_feat_val_two_class_2.pkl", 'rb') as param_dump:
            self.__feature_validity = cPickle.load(param_dump)

        with open("dumps/gait_mue_two_class_2.pkl", 'rb') as param_dump:
            self.__std_mue = cPickle.load(param_dump)

        with open("dumps/gait_sigma_two_class_2.pkl", 'rb') as param_dump:
            self.__std_sigma = cPickle.load(param_dump)

    def binary_classify_data(self, ):

        #parameter_candidates = [
        #    {'C': np.linspace(5, 50, 20), 'gamma': np.logspace(-5, -3, 15), 'kernel': ['rbf']},
        #]
        ...
        self.__clf = svm.SVC(C=12, kernel='rbf', gamma=0.0002).fit(self.__training_data, self.__training_labels)
        self.__clf.fit(self.__training_data, self.__training_labels)
    ...
    def test_new_data_set(self, file_source_name, start_index=0, stop_index=10**16, label=None, plot_output=False):
        ...
        if plot_output:
            plt.figure()
            for j in range(len(data_set[0][0, ::])):
                plt.plot(data_set[0][::, j], label=str(j))
            title = "(test_new_data_set) Test data check... "
            plt.title(title)
            plt.legend()
            plt.show()
        ...
        x_axis = np.linspace(start_index, stop_index, stop_index-start_index)
        ...
        for i in range(len(test_features_std_temp)):
            ...
            plt.plot(x_axis[i*self.__window_shift:(i*self.__window_shift+self.__window_width)],
                     data_set[0][i*self.__window_shift:(i*self.__window_shift+self.__window_width), 0],
                     color='#336600', label='0')
            plt.plot(x_axis[i*self.__window_shift:(i*self.__window_shift+self.__window_width)],
                     data_set[0][i*self.__window_shift:(i*self.__window_shift+self.__window_width), 1],
                     color='#33ff00', label='1')
            plt.plot(x_axis[i*self.__window_shift:(i*self.__window_shift+self.__window_width)],
                     data_set[0][i*self.__window_shift:(i*self.__window_shift+self.__window_width), 2],
                     color='#990033', label='2')
            plt.plot(x_axis[i*self.__window_shift:(i*self.__window_shift+self.__window_width)],
                     data_set[0][i*self.__window_shift:(i*self.__window_shift+self.__window_width), 3],
                     color='#229911', label='3')
            plt.plot(x_axis[i*self.__window_shift:(i*self.__window_shift+self.__window_width)],
                     data_set[0][i*self.__window_shift:(i*self.__window_shift+self.__window_width), 4],
                     color='#ee1100', label='4')
            plt.plot(x_axis[i*self.__window_shift:(i*self.__window_shift+self.__window_width)],
                     data_set[0][i*self.__window_shift:(i*self.__window_shift+self.__window_width), 5],
                     color='#ff5500', label='5')
            if pred == 0:
                plt.axvspan(i*self.__window_shift, (i*self.__window_shift+self.__window_width), facecolor=(1, 1, 0),
                            alpha=0.5)
        font = {'family': 'normal',
                'size': 20}
        ...
        plt.xlabel('samples')
        plt.ylabel('U in LSB')
        plt.legend(['2y0a02', '2y0a02 (!)', '2y0a21', '2y0a21', '0a41sk', '0a41sk'])
        plt.show()
...
if __name__ == '__main__':
    oc_clf = Two_class_svm(window_width=2**6,
                           window_shift=2**4,
    ...
    '''
    # normal:
    oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=6500, stop_index=7500, label=0)
    ## oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=37000, stop_index=47000, label=0)
    oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=37500, stop_index=38500, label=0)
    oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=45500, stop_index=47000, label=0)
    oc_clf.add_data_files("2019_1_15_18_4_Outdoor.txt", start_index=5500, stop_index=8500, label=0)
    ...
    oc_clf.add_data_files("2019_1_15_18_4_Outdoor.txt", start_index=3000, stop_index=4000, label=0)
    oc_clf.add_data_files("2019_1_15_18_4_Outdoor.txt", start_index=6000, stop_index=8000, label=0)
    oc_clf.add_data_files("2019_1_15_18_4_Outdoor.txt", start_index=12000, stop_index=13000, label=0)
    oc_clf.add_data_files("2019_1_15_18_4_Outdoor.txt", start_index=16000, stop_index=17000, label=0)
    # oc_clf.add_data_files("2019_1_30_13_45.txt", start_index=108000, stop_index=110000, label=0)
    oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=1000, stop_index=7500, label=0)
    oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=14000, stop_index=17000, label=0)
    oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=18000, stop_index=20000, label=0)
    oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=22000, stop_index=25000, label=0)
    # oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=21000, stop_index=26000, label=0)                   # viel
    oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=36400, stop_index=37000, label=0)
    oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=37600, stop_index=38600, label=0)
    oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=40000, stop_index=41000, label=0)
    # oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=36000, stop_index=46000, label=0)                   # viel
    oc_clf.add_data_files("2019_3_5_9_21.txt", start_index=1500, stop_index=3300, label=0)       #
    oc_clf.add_data_files("2019_3_5_9_21.txt", start_index=4000, stop_index=5500, label=0)       #
    oc_clf.add_data_files("2019_3_5_9_21.txt", start_index=9000, stop_index=10200, label=0)       #
    oc_clf.add_data_files("2019_3_5_9_21.txt", start_index=15500, stop_index=17000, label=0)       #
    oc_clf.add_data_files("2019_3_5_9_21.txt", start_index=18000, stop_index=18500, label=0)       #
    oc_clf.add_data_files("2019_3_5_9_21.txt", start_index=89000, stop_index=89400, label=0)       #
    oc_clf.add_data_files("2019_3_5_9_21.txt", start_index=92000, stop_index=93200, label=0)       # Thomas (evtl. Faxen)
    oc_clf.add_data_files("2019_3_5_9_21.txt", start_index=98000, stop_index=99750, label=0)       #
    oc_clf.add_data_files("2019_3_5_9_21.txt", start_index=100200, stop_index=101000, label=0)       #
    oc_clf.add_data_files("2019_3_5_9_21.txt", start_index=101700, stop_index=107600, label=0)       #
    oc_clf.add_data_files("2019_3_7_10_24.txt", start_index=7750, stop_index=8000, label=0)       # schwarze Hosen
    oc_clf.add_data_files("2019_3_7_10_24.txt", start_index=8250, stop_index=8500, label=0)       # schwarze Hosen
    oc_clf.add_data_files("2019_3_7_10_24.txt", start_index=8800, stop_index=9200, label=0)       # schwarze Hosen
    oc_clf.add_data_files("2019_3_7_10_24.txt", start_index=9500, stop_index=9850, label=0)       # schwarze Hosen
    oc_clf.add_data_files("2019_3_7_10_24.txt", start_index=10300, stop_index=11100, label=0)       # schwarze Hosen
    oc_clf.add_data_files("2019_3_7_10_24.txt", start_index=17300, stop_index=18300, label=0)       # Daniel H.
    # not normal:
    oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=30600, stop_index=31200, label=1)
    oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=31400, stop_index=31600, label=1)
    oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=47500, stop_index=48000, label=1)
    oc_clf.add_data_files("2019_1_15_17_7_Gehsteig.txt", start_index=19600, stop_index=20200, label=1)
    oc_clf.add_data_files("2019_1_15_17_7_Gehsteig.txt", start_index=22000, stop_index=25000, label=1)
    oc_clf.add_data_files("2019_1_15_17_7_Gehsteig.txt", start_index=400, stop_index=1000, label=1)
    oc_clf.add_data_files("2019_1_15_18_4_Outdoor.txt", start_index=100, stop_index=800, label=1)
    oc_clf.add_data_files("2019_1_15_18_4_Outdoor.txt", start_index=29400, stop_index=30500, label=1)
    # oc_clf.add_data_files("2019_1_30_13_45.txt", start_index=28000, stop_index=36000, label=1)     #
    # oc_clf.add_data_files("2019_1_30_13_45.txt", start_index=169000, stop_index=170000, label=1)   # difficult
    # oc_clf.add_data_files("2018_11_22_10_29.txt", start_index=0, stop_index=90000, label=1)        #
    # oc_clf.add_data_files("2018_12_20_8_19.txt", start_index=0, stop_index=400000, label=1)        #
    oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=31500, stop_index=32500, label=1)       #
    oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=34600, stop_index=35000, label=1)       #
    oc_clf.add_data_files("2019_1_15_16_52.txt", start_index=35600, stop_index=36000, label=1)       #
    #oc_clf.add_data_files("2019_3_5_9_21.txt", start_index=93400, stop_index=94100, label=1)       # Thomas (Faxen gemacht)
    oc_clf.add_data_files("2019_3_5_9_21.txt", start_index=21750, stop_index=23000, label=1)       #
    oc_clf.add_data_files("2019_3_5_9_21.txt", start_index=97000, stop_index=98000, label=1)       #
    oc_clf.add_data_files("2019_3_7_10_24.txt", start_index=11500, stop_index=11800, label=1)       # schwarze Hosen
    oc_clf.add_data_files("2019_3_7_10_24.txt", start_index=9250, stop_index=9450, label=1)       # schwarze Hosen
    oc_clf.add_data_files("2019_3_7_10_24.txt", start_index=1100, stop_index=1200, label=1)       # schwarze Hosen
    oc_clf.add_data_files("2019_3_7_10_24.txt", start_index=4300, stop_index=4800, label=1)       # schwarze Hosen
    '''
    ...
    oc_clf.binary_classify_data()
    oc_clf.dump_all()
    '''
    oc_clf.load_all()
    #oc_clf.test_new_data_set(file_source_name="2018_11_22_13_28_vorstudien.txt", start_index=2000, stop_index=12000,
    #                         plot_output=False)
    oc_clf.test_new_data_set(file_source_name="2018_11_22_13_45_vorstudien.txt", start_index=16000, stop_index=18000,
                             plot_output=False)
